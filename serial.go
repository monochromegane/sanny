...
func IntsToBytes(ints []int) []byte {
	bytes := make([]byte, len(ints)*SIZE_FLOAT32)
	for i, _ := range ints {
		binary.BigEndian.PutUint32(bytes[SIZE_FLOAT32*i:SIZE_FLOAT32*i+SIZE_FLOAT32], uint32(ints[i]))
	}
	return bytes
}
...
func BytesToInts(bytes []byte) []int {
	...
	ints := make([]int, size)
	for i := 0; i < size; i++ {
		ints[i] = int(int32(binary.BigEndian.Uint32(bytes[0:SIZE_FLOAT32])))
